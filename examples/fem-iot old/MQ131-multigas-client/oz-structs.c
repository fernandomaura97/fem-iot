...
multigas_values_t sensor_values;
...
{ 
   ...
   sensor_values.ppm = MQ_getO3(PPM);

   i2c_master_enable();
   sensor_values.co = measure_CO();
   sensor_values.no2 = measure_NO2(); 
  

   /*
   if(dht22_read_all(&sensor_values.temperature, &sensor_values.humidity) != DHT22_ERROR) {
      printf("read ok\n");
    } else {
      printf("Failed to read the sensor\n");
      //sensor_values.temperature = dht22.value(DHT22_READ_TEMP);
      //sensor_values.humidity = dht22.value(DHT22_READ_HUM);
  ...
  if(is_reachable && root_found) {
    ...
    LOG_INFO_("\n");
     ...
     ////EXPERIMENTAL, PROBAR///
        ...
        simple_udp_sendto(&udp_conn, &sensor_values, sizeof(sensor_values), &dest_ipaddr);

...
while(1) {
  ...
  //Sample Multichannel gas sensor
    ...
    snprintf(co_, sizeof(co_),"\"co1\": %d.%s", (int)c__, &fractional_part[1]);
    ...
    sprintf(fractional_part, "%d", (int)((c__ - (int)c__ + 1.0) * 1000));
    snprintf(no2_, sizeof(no2_),"\"no21\": %d.%s", (int)c__, &fractional_part[1]);
  ...
  if(loudness != ADC_WRAPPER_ERROR) {
      printf("%u\n", loudness);
      snprintf(louds, sizeof(louds), "\"Noise1\": %u", loudness);
  } else {
...
pm10_value = pm10.value(1);
  ...
  if(pm10_value != ADC_WRAPPER_ERROR) {
    ...
    snprintf(pm10s, sizeof(pm10s), "\"PM101\": %u", pm10_value);
} else {
    printf("Error, enable the DEBUG flag in adc-wrapper.c for info\n");
    PROCESS_EXIT();
}
